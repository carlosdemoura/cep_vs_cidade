...
Uso:
    ...
    python correios_cep.py --captcha tesseract  # resolve o captcha com OCR automático
"""
...
# ---------------------------------------------------------------------------
# Captcha — captura a imagem do browser e entrega ao solver escolhido
# ---------------------------------------------------------------------------

def _screenshot_captcha(driver: webdriver.Chrome) -> Path:
    """Tira screenshot do elemento captcha e salva num arquivo temporário."""
    tmp = Path(tempfile.mktemp(suffix=".png"))
    try:
        driver.find_element(By.ID, "captcha_image").screenshot(str(tmp))
    except NoSuchElementException:
        driver.save_screenshot(str(tmp))
    return tmp


def _solve_manual(driver: webdriver.Chrome) -> str:
    """Solver manual: mostra o caminho da imagem e pede digitação."""
    tmp = _screenshot_captcha(driver)
...
def _solve_tesseract(driver: webdriver.Chrome) -> str:
    """Solver automático via pytesseract + Pillow.
    ...
    try:
        ...
        raise ImportError(
            ...
            "Sistema: sudo apt install tesseract-ocr  |  brew install tesseract"
        )

    tmp = _screenshot_captcha(driver)
    try:
        img = Image.open(tmp).convert("L")                    # escala de cinza
        img = img.filter(ImageFilter.SHARPEN)                 # nitidez
        img = img.point(lambda p: 255 if p > 140 else 0)     # binariza
        img = ImageOps.invert(img)                            # texto preto, fundo branco
        ...
        print(f"\n[CAPTCHA] Tesseract leu: '{result}'")
        return result
    finally:
        tmp.unlink(missing_ok=True)


def _build_solver(mode: str):
    """Retorna a função solver adequada ao modo."""
    if mode == "tesseract":
        return _solve_tesseract
...
def _scrape_uf(uf: str, driver: webdriver.Chrome, solver_fn, retries: int, page_delay: float) -> list[dict]:
...
def scrape_cep(
    ...
    Args:
        ...
        captcha:    Modo de resolução: "manual" (padrão) ou "tesseract".
    ...
    solver_fn = _build_solver(captcha)
    driver = _make_driver(headless=headless)
    all_records: list[dict] = []

    try:
        for uf in target_ufs:
            records = _scrape_uf(uf, driver, solver_fn, retries=retries, page_delay=page_delay)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--captcha", choices=["manual", "tesseract"], default="manual",
                        help="Modo de resolução do captcha (padrão: manual).")
